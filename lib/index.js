 ...
 */
import {base64Decode, base64urlDecode} from './util.js';
...
// matches an absolute-URL string, i.e. one that starts with a scheme; per
// the WHATWG URL standard a scheme is one ASCII alpha followed by zero or
// more ASCII alphanumerics, `+`, `-`, or `.`, then `:`
...
export async function verifyCapabilityInvocation({
      ...
      try {
        capability = JSON.parse(
          new TextDecoder('utf-8').decode(
            pako.ungzip(base64urlDecode(capability))));
      } catch(e) {
  ...
  // because it is common to do so despite non-conformance); instead,
  // check for a scheme at the start of the URL
